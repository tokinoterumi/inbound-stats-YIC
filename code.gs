...
function getAllCountryData() {
  ...
  if (!countriesSheet) {
    throw new Error(
      "The 'Countries' sheet was not found. Please ensure it exists."
    )
  }
...
/**
 * Retrieves all entries from the past 14 days.
...
function getRecentEntries() {
  ...
  fourteenDaysAgo.setHours(0, 0, 0, 0) // Start of the 14-day window
  ...
  // If the 14-day window crosses the fiscal year boundary (April 1st), add the previous year's sheet
...
function editEntry(timestampStr, updatedEntry) {
  ...
  if (!sheet)
    throw new Error(
      `Data sheet for the corresponding year (${sheetName}) not found.`
    )

  const textFinder = sheet
    .getRange("A:A")
    .createTextFinder(timestampStr)
...
function calculateMonthlySummary(year, month_0_based) {
  ...
  for (let i = 1; i < countryData.length; i++) {
    // Assumes headers on row 1 of Countries sheet
  ...
  if (dataSheet) {
    ...
    if (allData.length > 2) {
      ...
      if (timestampIdx === -1 || countryIdx === -1 || visitorsIdx === -1) {
        throw new Error(`Required headers not found in ${dataSheetName}`)
      }
      ...
      for (let i = 0; i < dataRows.length; i++) {
        ...
        if (
          ...
          timestamp.getMonth() === month_0_based
        ) {
          const countryName = row[countryIdx]
          const visitors = Number(row[visitorsIdx]) || 0
          monthlyTotals[countryName] =
            (monthlyTotals[countryName] || 0) + visitors
        }
      }
    }
  }
  ...
  for (const countryName in countryInfo) {
    ...
    if (!summaryByRegion[region]) {
      ...
      if (region !== "Unknown") {
        ...
        summaryByRegion[region][restKey] = {
          country: restKey,
          visitors: 0,
          isFrequent: false,
          order: 9999,
    ...
    if (info.isFrequent) {
      summaryByRegion[region][countryName] = {
        country: countryName,
        visitors: visitors,
        isFrequent: true,
        order: info.order,
  ...
  for (const region in summaryByRegion) {
    finalOutput[region] = Object.values(summaryByRegion[region]).sort(
      (a, b) => a.order - b.order
    )
  }
  ...
  if (fiscalYear === "All") {
    ...
    for (const sheet of allSheets) {
      ...
      for (const sheet of sheetsToSearch) {
        const textFinder = sheet
          .getRange(INQUIRY_COLUMN_LETTER + ":" + INQUIRY_COLUMN_LETTER)
